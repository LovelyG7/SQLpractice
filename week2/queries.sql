...
-- until you reach the final result.
 ...
 WHERE units_in_stock <= reorder_level AND discontinued != TRUE  AND units_on_order > 0
 ORDER BY product_id;
...
--WHEN DO I USE THIS us_states.state_abbr = customers.region.
SELECT s.state_abbr, c.company_name, s.state_name
FROM us_states s
...
SELECT company_name,
SUM(quantity) FROM order_details od
